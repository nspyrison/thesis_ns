...
This current and future work is done with reproduction priciples in
...
Directories are hosted publically on GitHub. Reproduction of this report
...
{\centering \includegraphics[width=4.99in]{./figures/data_analysis_workflow} 

}

\caption{Data analysis workflow. This research
aids visualisation in exploratory data analysis and in the workflow.}\label{fig:dataanalysisworkflow}
\end{figure}
...
tasks wihtin the different modalities, comparing biometric readings, and
subjective likert survey of paricipents. A lineup-type model as outlined
in \textcite{hofmann_graphical_2012} may also be employed for assessing
quality of display types.

In this report, the related liturature is discussed in chapter
...
vizualization can be found in appendix \ref{sec:tour_notation}, and
except of a paper draft to be subbmitted to the R Journal can be found
in \ref{ch:spinifex_paper}.

\chapter{Literature review}\label{ch:lit_review}

In the section below we discuss the acedemic research in two primary
areas: that of dynamic linear projection (collectively known as tours)
followed by multivariate data visualisation in stereoscopic 3D.
...
reimplemented with an eye on extensiblity and maintanability,
...
dynamic linear projections stands in theory, but a direct comparison
with popular alternatives should be made. Barriers to addoption should
...
dynamic projections (\textbf{RO \#3}). After the implementation The
eficacy of \(d=3\) tours should be compared across display device
...
precede RO \#4, exploring the eficacy of 3D UCS accross display devices.
...
{\centering \includegraphics[width=0.7\linewidth]{./figures/ProjectOverview} 

}

\caption{Flow chart of research objective (RO)
dependancies, work order, and methodology.}\label{fig:ProjectOverview}
\end{figure}

\chapter{Completed work}\label{ch:completed_work}

Implementing UCS in low dimensions is preliminary to the extention of
the UCS into 3D space. This contribution (C \#1) answers the RO, and
forms the foundation for future work adressed by RO \#3 \& 4.
...
\item
  ...
  to 1 before returning to the inital position.
\end{enumerate}
...
\ref{sec:algorithm} and is purposfully removed here.}

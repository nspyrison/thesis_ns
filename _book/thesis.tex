...
the data when dimensions at centered, but not sphered (6 components
explains 53\% of sphered data, sphered data not used below). Data are
colored by experiment type with `dimuon SIDIS' in green , `DIS HERA1+2'
...
\caption{\label{fig:PDF7good}FILL CAPTION HERE}
\end{figure}
...
This article was created using Rmarkdown (\textcite{xie_r_2018}), with
code to generate the examples inline, and the source files can be found
at
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{print}\NormalTok{(}\StringTok{"opts: eval=F, echo=T"}\NormalTok{)}
...
{[}1{]} ``opts: results=`asis'\,'' {[}1{]} 2 {[}1{]} ``opts:
results=`asis'\,'' {[}1{]} 2

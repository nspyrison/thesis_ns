...
for quite some time. \autocites{PCA}{LDA-FISCHer1936} and all single
value deconcomposistion (SVD) techniques from numerous disciplines use
such embeddings. Previous application look at data in one (or few)
static orientations, after some objective optimization. For instance in
PCA, we reorient \(p\)-dimensions such that we ordered components that
describe a descending ammount of variation held within the data. Yet we
still have \(p\) components remaining, \(p\) dimensions to visualize.
Where does the reduction come from? \ldots{}
...
\autocite{tSNE}, building off of sammon mappings \autocite[
mappings]{sammon}. \ldots{}
...
of an XYZ Caresian volume (\emph{ie.} a basis \(\in \mathbb{R}^p\)),
each at a right angle and pointing 1 unit in each direction. In
...
could not find function ``render\_slideshow''..

...
of (\(p _gt 3\)) multivariate data sets. This paper discusses the
\emph{R} package \emph{spinifex}, which adds support for the manual
tour, which is particularly usefuly for exploring the local structure
after identifying a feature of interest, perhaps via guided tour.
...
\section{Other content:}\label{other-content}

\begin{itemize}
\tightlist
\item
  Supply an orthonormal basis, \textbf{B}\_\{{[}p x d{]}\}. Let \emph{d
  = 2} for illustration.
\item
...
\textbf{M}\emph{\{{[}p, d+1{]}\} by a rotation matrix, \textbf{R}}\{d+1,
d+1\} producing as many basis-projections, \textbf{P}\_\{b{[}p,
d+1{]}\}.
...
\item
  ...
  data, \textbf{X}\emph{\{{[}n x p{]}\}, for \textbf{P}}\{d{[}n x
  d+1{]}\}.
\end{itemize}
...
\chapter{\texorpdfstring{\texttt{\{r\}\ \#\ library(spinifex)\ \#\ flea\_std\ \textless{}-\ tourr::rescale(tourr::flea{[},1:6{]})\ \#\ rb\ \textless{}-\ tourr::basis\_random(n\ =\ ncol(flea\_std))\ \#\ \ \#\ spinifex(data\ =\ flea\_std,\ basis\ =\ rb,\ manip\_var\ =\ 4)\ \#}}{\{r\} \# library(spinifex) \# flea\_std \textless{}- tourr::rescale(tourr::flea{[},1:6{]}) \# rb \textless{}- tourr::basis\_random(n = ncol(flea\_std)) \#  \# spinifex(data = flea\_std, basis = rb, manip\_var = 4) \#}}\label{r-libraryspinifex-flea_std---tourrrescaletourrflea16-rb---tourrbasis_randomn-ncolflea_std-spinifexdata-flea_std-basis-rb-manip_var-4}

\chapter{\texorpdfstring{could not find function
``render\_slideshow''..}{could not find function render\_slideshow..}}\label{could-not-find-function-render_slideshow..}

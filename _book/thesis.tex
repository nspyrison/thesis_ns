...
Studies in 3D visualization generally show increased accuracy and
perception of visuals in 3D over 2D. Dynamic linear 2D projections have
been viewing briefly in head-tracked VR, but never with true 3D
projections. The Immersive Analytics Toolkit offers a frame works
available for implementing data visualization in VR.

This research involves extending data visualization as data
dimensionality increases, improving the perception of dynamic linear
projections via application in VR, and novel dynamic projections in 3D
space, and explores the potential gains from doing so.
...
visualization can be found in appendix \ref{sec:tour_notation}, and
except of a paper draft to be submitted to the R Journal can be found in
\ref{ch:spinifex_paper}.

\chapter{Literature review}\label{ch:lit_review}

In the section below we discuss the academic research in two primary
areas: that of dynamic linear projection (collectively known as tours)
followed by multivariate data visualization in stereoscopic 3D.
...
In the introduction we have established that visualizing data-space is
an important aspect of exploratory data analysis and data analysis in
general. Yet, there is an inherent difficulty as the dimensionality of
data increases. In univariate data sets histograms, or smoothed density
...
Implementing UCS in low dimensions is preliminary to the extension of
the UCS into 3D space. This contribution (C \#1) answers the RO, and
forms the foundation for future work addressed by RO \#3 \& 4.

...
2D monitors, and most commonly viewed as a projection down to \(d=2\).
The exception being \textcite{nelson_xgobi_1998}, where \(d=3\) tours
were viewed in 3D head-tracked VR. Data visualization studies generally
show an improved perception of 3D visuals over 2D, especially when
adequate depth cues are provided. The state of modern hardware
...
\item
  ...
  \ref{ch:future_work}.) The UCS algorithm needs to be extended to
  arbitrary, or at least, a third dimension and the rendering of 3D
  tours for use across mixed reality display devices. This involves
  communication between programming languages and allows for the
  comparison across display devices.
\item
  \textbf{Does UCS in 3D displays provide perception benefits over 2D
  displays?} (Future work, chapter \ref{ch:future_work}.)The addition of
  a stereoscopic 3rd dimension has previously shown improved perception.
  Building on the work from the previous objective the efficacy of 3D
...
work in RO \#1 will be extended to the third dimension, if not arbitrary
dimensions. Surface and function projections will also be developed.
This forms the calculation base for the work. After the projections have
been produced, they will be used consumed by the immersive analytics
toolkit (IATK) \autocite{cordeil_immersive_2019}, an extensible toolkit
for visualizing data in VR. This standardized interface across display
devices.
...
across various display devices. In this design, every participant will
complete every task on every display device. Display device is the
design variable with task type as the covariate. Quantitative
...
spinifex will be extended to 3D, and surface \& function projections
will be implemented. After the projections are performed \emph{Unity}
will be used to render the embeddings in 3D VR and offer a compatible
...
task on every display device. Display device is the design variable with
task type as the covariate. Task order and display device will be

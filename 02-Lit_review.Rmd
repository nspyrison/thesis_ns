...
* For projections down to 1- and 2-d, It's common to display the contribution and direction on it's own axis (1d) or relative to a unit circle (2d), this is sometime refereed to as basis axes or a reference frame.
...
<!-- After target bases are identified, the frames inbetween need to be filled in. There are several methods to do so: -->
<!-- * Geodesic -->
...
The little tour introduced in @mcdonald_interactive_1982, on the the other hand is necessarily both dense and rapid, performing essentially an exhaustive search on the Grassmannian. However this path uninteresting and with long periods of similiar projections strung together. There was a need to find interesting projections quicker.

Guided tours (@hurley_analyzing_1990) optimize an ojective functionm generating path will be relatively small subset of the Grassmanian, density and rapidity become poor measures, while interesting projections are quickly identified. Recently, @laa_using_2019, compares projection pursuit indices with the metrics: smoothness, squintability, flexibility, rotation invariance and speed. See the original work to see how the indices perform.
...
* 2d geoms
    ...
    + $x-y$ scatterplot
* 2.5d geoms - these geoms do not perform project to 3 dimensions, but rather give persception cues into the add dimension of the manipulation space of 2d projection.
...
* ORCA [@sutherland_orca_2000] -- Extensible toolkit build off of object oriented Java.
...
## Data vizualization in 3D

For the sake of brievity we'll stick to discussion of multivariate data vizualization in 3D, for wider scope see chapter 2 of @marriott_immersive_2018.

### Terminology

* 2D - representation of data in strictly 2 dimensions, without any indiction to other information.
* 2.5D - any of the many degrees between 2- and 3D. Including asthetic mapping (colors/size) to data points and use of some incomplete combination of depth cues. 
* 3D - stereoscopically true display of information with all depth cues in tack.
* Depth cues - an indication that tips off depth to an observer, including: linear perspective, aerial perspective, occulation, motion perspectice, accommodation, convergance, binocular disparity, and binocular stereopsis
...
Scientific visualization have readily adopted mixed realities as a large amount of the science exist in 3 spatial dimensions, lending itself well to virtual immersion. Data vizualisation, on the other hand, has been been slow to utilize graphics above 2.5D, (and haptic interaction) primarily due to the mixed results of over-hypde of 3D visuals from the 1980's and 90's [@munzner_visualization_2014]. However, since then there have been a number of promising studies suggesting that it's time for data vizualiazation to revisit and adopt 3D visuals for specific combinations of visuals and depth cues.

### 3D rotated projections vs 3 2D orthogonal projections 

3D shapes can be represented by 3 orthogonal 2D views, or rather 3 pairwise projections. When 3D representations are used with binocular cues they are found to have more accurate perception than 2D counterparts [@lee_effects_1986]. Using 3D rotated projection gives accurate preception of a ball relative to complex box shapes, while combinations of 2D and 3D give the most precice orientation and positioning information [@tory_visualization_2006]. Between 3D and split view 2D of 3-dimensional economics data @wickens_implications_1994 asked participants integrative questions, fiding that participants were faster to answer when questions involves three dimension, while performance was similar when questions involved fewer dimensions. 

### 3D and 2D projections of multivariate data
<!-- @wegman_immersive_2002 : "visual data mining"-->

@nelson_xgobi_1998 had participants perform burshing and touring tasks (indentification of clusters, structure, and data dimensionality) in 3D with head-tracked binocular VR. 3D proved to have substaintial advantage for cluster identification and some advantage in in identifying shape. Brushing did take longer in VR, perhaps due to the lower familiarity of manipulating 3D spaces. Another study, @gracia_new_2016, performed dimensionality reduction down to 2- and 3-D scatterplots, both displayed in monocular 3D on a standard moniter. Users were found to more accurately compare distances between points and identify outliers on 3D scatterplots. Howeveer, both tasks were performed slower wiht use of the 3D scatterplots and statitically significance was not reported.

@sedlmair_empirical_2013: SCATTERPLOT MATRIX (SPLOMS)

...
I order to increase the speed of finding insteresting projections @hurley_analyzing_1990 implemented guided tours. The optimization of the ojective function means path of projection pursuit will be relatively small, density and rapidity become poor measures, but interesting projections are quickly identified. Recently, @laa_using_2019, compares projection pursuit indices with the metrics: smoothness, squintability, flexibility, rotation invariance and speed. 
...
<!-- The index values for most PPIs increases -->
<!-- substantially nearing the structured projection, indicating that they \see" -->
<!-- the structure. Some indexes see all three structures: scagnostics, MIC and -->
<!-- TIC, which means that they are  -->
<!-- exible indexes capable of detecting a range -->
<!-- of structure. (Grimm 2016)'s indexes, dcor2d and splines2d, are -->
...
<!-- squint angle are dicult to nd, because the optimization algorithm -->
<!-- needs to get very close to begin hill-climbing to the optimum. The -->
<!-- analyst doesn't have control over the data structure, but does have -->
...
<!-- projections as dierent from noise. To examine squintability, the PPI -->
<!-- values are examined on interpolated tour paths between a noise projection -->
<!-- and a distant structured projection. -->

<!-- exibility: An analyst can have a toolbox of indices that may cover the -->
<!-- range of ne and broad structure, which underlies the scagnostics suite. -->
<!-- Early indexes, based on density estimation could be programmed to detect ne or large structure by varying the binwidth. This is examined -->
...
<!-- dened between planes, along a geodesic path, not bases within planes. -->
...
<!-- describes alternative interpolation paths based on Givens and Householder -->
<!-- rotations which progress from basis to basis. It may be possible -->
...
<!--  speed: Being fast to compute allows the index to be used in realtime -->
<!-- in a guided tour, where the optimization can be watched. When -->
<!-- the computations are shifted o-line, to watch in replay, computation -->

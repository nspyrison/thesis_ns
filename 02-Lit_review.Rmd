...
In univariate data sets histograms, or smoothed density curves are employed to visualize data. In bivariate data $x-y$ scatterplots and contour plots (2D density) can be employed. In three dimensions the two most common techniques are 3D scatterplot\footnote{Graphs depicting 3 dimensions are typically printed on paper, or rendered on a 2D monitor, they are intrinsically 2D images of monocular 3D spaces, sometimes referred to as 2.5D, but more frequently referred to as 3D, more on this in section \ref{sec:3d-terminology}.} or 2D scatterplot with the 3rd variable as an aesthetic (such as, color, size, height, $etc.$). Mapping varibles to data point aesthetics can afford another dimension or 2, but this is not a sustainable solution.

How do we visualize data for even modest values of $p$ numeric dimension (say 6 or 12)? It's far too common that visualizing in data-space is dropped altogether in favor of modeling parameter-space, model-space, or worse: long tables of statistics without visuals [@wickham_visualizing_2015]. Yet, there are inherant risks inherent in relying too heavily on summary statistics alone [@anscombe_graphs_1973; @matejka_same_2017]. So why do we move away from visualizing in data-space? Scalability, in a word. Humans are not equiped to visualize above 3 spatial dimensions. This is where dimensionality reduction comes in. We will focus on a group of linear othogonal projection techniques know as touring. For a broader view of dimensionality reduction techniques, see @grinstein_high-dimensional_2002, @carreira-perpinan_review_1997, or @heer_tour_2010. We choose touring, for a couple of salient features: it use linear projections maintaining transparency back to the original variable space (which non-linear projections lose) and does not discard components and their contained information (which static linear projections lose). By employing the breadth of tours, we can preserve the visualization of data-space, and with it, the intrinsic understanding of structure and distribution of data that is more succinct or beyond the reach of statistic values alone.
...
The grand and little tour have no input from the user aside from the starting basis. Guided tours allows for an index to be selected, but the bulk of touring development since has largely been around dynamic display, user interaction, geometric representation, and application. The extent to which will be expounded on in the following sections.
...
* 2D geoms
    ...
    + $x-y$ scatterplot
* 3D geoms - these geoms do not perform projections in 3 dimensions, but rather are $d=2$ projections that utilize the the manipulation dimension to give depth perception cues.
...
As this research pertains to numeric multivariate data, we'll stick to this scope in the liturature review. For wider overview of 3D data visualization see chapter 2 of @marriott_immersive_2018.
...
* 2.5D - Following the definition given in @ware_designing_2000 : visualisations that are essentially 2D, but select depth cues are used to provide some suggestion of 3D. However, the term 2.5D is commonly used for several meanings *due to the ambiguity use of 2.5D, we err on the side stating 3D with descriptions of depth cues used*.

...
We have discussed a number of different multivariate visualizations. The analyst is left  to determine which visualization to use. In Chapter \@ref(ch-userstudy) conduct a user study comparing competing visualizations.

<!-- Related studies & research gap -->
There have been several user studies comparing scatterplots of dimension-reduced spaces across 2D and 3D scatterplots on traditional 2D monitors [@gracia_new_2016; @wagner_filho_immersive_2018]. There are also empirical statistics used to describe non-linear reduction and how well and faithfully they embed the data in a lower space [@bertini_quality_2011; @liu_visualizing_2017; @sedlmair_empirical_2013; @van_der_maaten_visualizing_2008]. @nelson_xgobi_1999 even compared 2D with 3D scatterplot tours for detecting clusters, and dependence, in multivariate data. However, there is an absence of studies comparing competing visual techniques.


## Non-linear models

In Chapter \@ref(ch-cheem) we turn our attention to predictive modeling, the lost of variable-level interpretablilty of the non-linear models, and an attempt to maintain model transparence with local eplaination.

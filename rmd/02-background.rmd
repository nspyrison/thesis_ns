...
(ref:ch2fig1-cap) Starting from the profile of a dinosaur, observations are allowed to drift (by iterated simulated annealing) toward a dozen patterns provided that they stay close to the original statistics [@matejka_same_2017]. Visualization of data yields stark designs that are easy to miss in numerical summarization.
...
## Multivariate data vizualization
...
<!-- scaling with n -->
Alternatively, as $n$ increases, scatterplots displays suffer from dense points start to occlude each other. This is typically addressed in a few ways. One method would decrease
points' opacity, allowing more layers to be seen. Less interestingly, visualization can be performed on a representative subset of the data. Another would change the geometric display to an aggregated heatmap or 2D density contours. These aggregations typically render faster and scale better with increasing observations. Moreover, these methods to alleviate observation density work more generally on any scatterplot display.
...
XXX Move these kinds of comments to end of related work >>>

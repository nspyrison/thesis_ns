...
(ref:penguinpcp-cap) Parallel coordinate plots put variables on a common scale and position them side-by-side with lines connecting observations. Some variation between the clusters can be seen, corroborating their importance in explaining cluster separation. This approach scales relatively well with the number of variables but poorly with the number of observations.
...
The same issues persist across displays that map observations into $n$ glyphs or pixel heatmaps. Examples of these include star plots [@chambers_graphical_1983], pixel-based visuals [@keim_designing_2000], and Chernoff faces [@chernoff_use_1973]. Like parallel coordinate plots, these other visuals scale quite poorly with increasing observations. However, because these visuals scale well with the number of variables, they may be candidate visualizations for low $n$ and high $p$ data.

...
<!-- Interpolation bases -->
Regardless of the type of tour, tours must interpolate intermediate bases between the distant generated target bases. This interpolation is performed along a geodesic path between the bases. Geodesic refers to the shortest path (on a $p$-sphere of possible bases). This ends up being slightly curved in 2D representation for the same reason that flight paths appear curved on maps. The interpolation of bases at small enough angles is foundational for the trackability of observations between frames.

<!-- Grand tour -->
Originally in a _grand_ tour [@asimov_grand_1985], several target bases are randomly selected. Figure \@ref(fig:ch2fig4) illustrates six frames of a grand tour. The grand tour is suitable for EDA in that it will show bases with widely varying contributions but lacks a destination, objective function, or means of steering.
...
<!-- Motivation -->
This chapter discussed the motivation for data visualization and the importance of user interaction. Several visuals were discussed before turning to linear dimension reduction. Empirical evaluations, nonlinear models, and their local explanations were also covered. There is an absence of studies comparing animated tours with alternative visualization. XAI and extending the interpretability of nonlinear models are sought-after.

...
The same issues persist across displays that map observations into $n$ glyph or pixel heatmaps. Examples of these include star plots [@chambers_graphical_1983], pixel-based visuals [@keim_designing_2000], and Chernoff faces [@chernoff_use_1973]. Like parallel coordinate plots, these other visuals scale quite poorly with increasing observations. However, because these visuals scale well with the number of variables, they may be candidate visualizations for low $n$, high $p$ data.


### Dimension reduction

<!-- Dimension reduction, linear and nonlinear -->
The other main approach for visualizing quantitative multivariate data is dimension reduction. This involves a function mapping $p$-spaces onto a lower $d$-dimensional space. Dimension reduction is separated into two categories, linear and nonlinear. The linear case spans all affine mathematical transformations, essentially any function where parallel lines stay parallel. Nonlinear transformations complement the linear case, think transformations containing exponents or interacting terms.
...
<!-- Interpolation bases -->
Regardless of the type of tour, tours must interpolate intermediate bases between the distant generated target bases. This interpolation is performed along a geodesic path between the bases. Geodesic refers to the shortest path (on a $p$-sphere of possible bases). This ends up being slightly curved in 2D representation in the same way that flight paths appear curved on 2D representations of the globe. The interpolation of bases at small enough angles is foundational for the trackability of observations between frames.

<!-- Grand tour -->
Originally in a _grand_ tour [@asimov_grand_1985], several target bases are randomly selected. Figure \@ref(fig:ch2fig4) illustrates six frames from a grand tour. The grand tour is suitable for EDA in that it will show bases with widely varying contributions but lacks a destination, objective function, or means of steering.
...
Nonlinear models have many or complex interactive terms, which cause an opacity to interpret the variables. This loss of the interpretability of variables in nonlinear models sometimes lead to them being referred to as black-box models.

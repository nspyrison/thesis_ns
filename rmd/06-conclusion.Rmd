...
Most data are multivariate. This thesis make important contributions to to the interactive visualization of data where all variables are quantitative. The methods are build on dynamic animation of linear projections, called tours. The thesis developed the radial tour, illustrating that it improved understand variable importance in higher dimensional data, and can be used to better understand black box models (XAI).
...
>>> link back RQ/contributions in introduction can add more detail

Chapter \@ref(ch-spinifex) develops radial tour methodology... <<< I clarify the use of Rodrigues' rotation formula to solve the rotation matrix with the definition of two manipulation angles. This sets up a scaffolding to extend the manual tour to three dimensions with another rotation angle to span the manipulation space. This work also supports the use of manual tours by illustrating use cases on high-energy physics data.
...
Below a number of limitations with this work are covered in chapter order. The following section will echo with possible directions to address these limitations. 

<!-- Manual tours in 1- & 2D -->
Manual tours (and radial tour) are limited in a number of ways. For instance, the work currently lists rotation matrices for 1 and 2D manual tours.<!-- only 1var at a time--> Another limitation is that they only change one variable at a time. This can be cumbersome and timely if many variable need to be zeroed changed in order.<!-- Ink tank display --> The radial tour's plays in three segments (full contribution-no contribution-initial contribution) takes a bit to unpack. It may be more approachable to directly relate the fraction of the slider to the magnitude of the variables contributions.

<!-- >> 8_Pedagogical side of manual tours for newcomers; not on limitation side, mention more on the future work side. -->

<!-- __spinifex, layered composition -->
<!-- 4_Layered tour composition --> 
Setting aside the manual tour, there are several extensions to the layered composition of tours in __spinifex__, such as extending the type of geometric display. Because the manual tour is currently defined for $d \in [1,2]$ projections the geometric display for $d>2$ __tourr__-made tours is under-supported.<!-- 5_Sterescopic 3D & XR --> Most tour implementations are made with 2D monitors in mind. 

<!-- 6_User study -->
The user sturdy evaluating the radial tour has a number of intrinsic limitations. It only considered discrete 2d PCA, grand tour, and the manual tour for supervised cluster separation for various levels of experimental factors effecting the data. 


<!-- 9_cheem, and model/explantion types -->
...
<!-- 11_cheem, scope/context of data -->
...
This section purposes possible directions to address or mitigate the limitations outlined above.

<!-- __Manual tours -->
<!-- 1_3D manual tour -->
Chapter \@ref(ch-spinifex) included the scaffolding for this task. Namely, this would require applying another angle of rotation with Rodrigues' rotation formula on the current 3D rotation matrix. Another display dimension may benefit the detection and understanding of the higher dimensional structure though the input of three angles may prove less natural and harder to capture.

<!-- 2_Manipulating multiple variables -->
In addition to the manual tour controlling the contribution of a single variable, it may be insightful to change the contributions of several variables at once (manipulating a linear combination). Alternatively, a sort of dimension reduction tour, appending several manual tours that sequentially zero the contributions of variables contributing less than some threshold, may prove to expedite analysis, especially for approaching a feature's intrinsic dimensionality.
...
<!-- __spinifex, layered composisition -->
<!-- 4_geom/protos for d>2 -->
Additional functions could be added to facilitate geometric display of parallel coordinate plots, glyph-based, and pixel-based visuals. These displays are potentially best used with data with relatively few observations and many variables.

<!-- 5_Layered tour composition --> 

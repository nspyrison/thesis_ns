...
Most data are multivariate. This thesis makes important contributions to the interactive visualization of quantitative multivariate data. Data visualization can convey complexities in data such as heteroskedasticity, irregular shapes of distributions, better than numerical summaries. Thus it is important activity for exploring data. However, it becomes more challenging when there are more than two variables (dimensions). It is common to tackle high-dimensions by reducing the dimensionality, and to visualize the lower dimensional data. This thesis provides new methodology using linear dimension reduction, built on dynamic animation of linear projections, with a specific approach called a radial tour. This has benefits over traditional approaches that use only a small handful of low-dimensional views, because many low-dimensional projections are shown. In particular, the radial tour is designed to help the user understand the relationship between a single variable and structure visible in the projections.

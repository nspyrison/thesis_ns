...
One of the most common and successful approaches to visualize multivariate data is to use linear projection, which approximates $p$-dimensional data onto 1-3 display dimensions. In the same way that a 3D object casts a 2D shadow, these projections cast one data orientation onto a lower plane.
...
__Can the radial tour, with user-steering of the basis, help analyst's understand the variable sensitivity of structure in the projection?__
...
At present, the radial tour is not used by analysts. Instead, they would use a single projection to understand the structure, almost always principal components analysis [PCA, @pearson_liii._1901], which chooses the basis that shows the most variation. Another approach is to use a _grand tour_ [@asimov_grand_1985]. The grand tour animates many interpolated frames between randomly selected target bases. Neither PCA nor the grand tour provides a means for manually manipulating a desired variable's contribution to the basis. We wish to investigate if the basis-steering of the radial tour facilitates a better understanding of variable sensitivity to the structure. 
...
The research corresponding to RQ 1 entails _algorithm & software design_ [@kleinberg_algorithm_2006], and adapts the algorithm from @cook_manual_1997.
...
- __cheem__, an __R__ package that facilitates the exploration of local explanations of nonlinear models through the radial tour.
    - Preprocessing: given a tree-based model, calculate the tree SHAP local explanation [via __treeshap__, @kominsarczyk_treeshap_2021] of all observations, and calculate statistics that accent the separability of this space.

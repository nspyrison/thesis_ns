...
## TODO: continue here.
<!-- Move storage and sharing up and rendering in R down? -->
### Rendering in R

The *tourr* package utilizes R's base graphics for the display of tours. *spinifex* allows tours to be used in rendered in `plotly` @sievert_plotly_2018 as an HTML5 object or`gganimate` @pedersen_gganimate:_2019 as .gif or .mp4 object. Both of which build from `ggplot2` plotting in internal functions.
...
Use the below code as a guide for installation and finding the vigette. The vignette is more lighter discussion and goes though a couple more use cases and examples. If you preffer to follow along with the example in the algorithm then simplified code is also listed below.

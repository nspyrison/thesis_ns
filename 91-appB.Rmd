...
Adding this extra dimension to our basis plane allows for the coefficients of the specified variable to be changed. For example, the ablility to lift a piece of paper, rather than being constrained to motion on a table top. Orthonormalizing rescales the new depth vector while the projection down to 2D is the original basis, that is the first $d$ vectors remain constant. Imagine the refererence axes (and projection plane) laying flat on a table, while a new dimension exisits with axes projecting back onto the reference axes. An illustration of such can be seen below in figure \@ref(fig:step2). The manip var is highlighted, while the depths of the other variales are not depicted. 

(ref:step2-cap) Manipulation space for controlling the contribution of `aede2` of standardized flea data. Basis selected by holes-index guided tour. The Projection plane is shown in blue. The manipulation axis, in red, allows the coeffiecients of the manip var to be changed.
...
A note on application: compile the sequence of $\phi_i$ and create an array/long table for each rotated manipulation space. $\phi$ is the angle relative to the initial value of $\phi$, we find the transformation $\phi_i$ - $\phi_1$ useful to think about $\phi$ relative to the basis plane. Additionally value of $\phi$ may be offset by a factor of pi. If the manip variable doesn't move as expected these are the first places to check.

...
Storing each data point for every frame of the animation is very inefficient. Just as operations are performed on the bases, so too should tour paths be stored as bases. Consider a radial manual tour, we can store the salient features in 3 bases, where $\phi$ is at its starting, minimum, and maximum values. The frames in between can be interpolated by supplying angular speed. With use of the `tourr::save_history()` function, the target bases can be saved. From there geodesic interpolation can be used to populate the intermitten frames. This type of interpolation should not be used on manual tours, which have already been initialized into a 3D manipulation space where direct linear interpolation is appropriate.

## TODO: continue here.

### Rendering in R

The *tourr* package utilizes R's base graphics for the display of tours. *spinifex* allows tours to be used in rendered in `plotly` @sievert_plotly_2018 as an HTML5 object or`gganimate` @pedersen_gganimate:_2019 as .gif or .mp4 object. Both of which build from `ggplot2` plotting in internal functions. Sharing on animations in not trivial especially in print and static formats such as .pdf. Even with the use of computers and dynamic file formats 

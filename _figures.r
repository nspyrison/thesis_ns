...
if (run) {
  #1_left
  (step1_l <- view_basis(h_bas)) # Maybe manual tour on v1?
  ...
  #=== Step 1 out
  (step1 <- grid.arrange(step1_l, step1_r, ncol=2))
  ggsave("./output/step1_basis+proj.png", step1, width = 4*1.61, height = 4)

  #=== Step 2 out
  (step2 <- view_manip_sp(h_bas, 5))
  ggsave("./output/step2_manip_sp.png", step2, width = 4, height = 4)
}

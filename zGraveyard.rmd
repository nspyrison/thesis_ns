...
## 2019_01_28 from zPDFSense_manualTour.R
...
interpolate2static <- function(.m_tour, .data, .cat, .mag =2.4) {
  ...
  gg2 <- gg1 +
    geom_point(data = data_grid,
               mapping = aes(x = V1 + x, y = V2 + y, color = species, pch = species)) +
    geom_text(data = data_grid,
              mapping = aes(x = x - .7, y = y + 1.1, label = paste0("frame: ",slide)),
              color = "grey50") +

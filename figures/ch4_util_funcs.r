...
pivot_longer_resp_ans_tbl <- function(dat){
  ...
  marks_longer <- dat %>%
    dplyr::select(key, sim_nm, v1_marks:v6_marks) %>%
    tidyr::pivot_longer(cols = v1_marks:v6_marks,
                        names_to = "var_num",
                        values_to = "marks",
                        values_drop_na = TRUE)
  signal_longer <- dat %>%
    dplyr::select(key, sim_nm, v1_signal:v6_signal) %>%
    tidyr::pivot_longer(cols = v1_signal:v6_signal,
                        names_to = "var_num",
                        values_to = "signal",
                        values_drop_na = TRUE)
  weight_longer <- dat %>%
    dplyr::select(key, sim_nm, v1_weight:v6_weight) %>%
    tidyr::pivot_longer(cols = v1_weight:v6_weight,
                        names_to = "var_num",
                        values_to = "weight",
                        values_drop_na = TRUE)
  accuracy_longer <- dat %>%
    dplyr::select(c(key, sim_nm, v1_accuracy:v6_accuracy)) %>%
    tidyr::pivot_longer(cols = v1_accuracy:v6_accuracy,
                        names_to = "var_num",
                        values_to = "accuracy",
                        values_drop_na = TRUE)
  if(all.equal(
    nrow(resp_longer), nrow(marks_longer), nrow(signal_longer),
    nrow(weight_longer), nrow(accuracy_longer)) == FALSE
  ) error("!!!all nrow() not equal!!!")
  ## cbind(), left_join not working.
  ret <- cbind(
    resp_longer, marks_longer[, 4L], signal_longer[, 4L],
    weight_longer[, 4L], accuracy_longer[, 4L]
  )
...
ggproto_ans_plot <- function(
  ...
  mark_col <- dplyr::if_else(sign(resp_ans_longer$accuracy) == 1L, "green", "red")
  ret <- c(ret,
           list(

...
Tours are typically viewed from standard 2D monitors, and most commonly viewed as a projection down to 2D. A notable exception being @nelson_xgobi_1998, where 3D embeddings were viewed in 3D head-tracked VR. Data visualization studies generally show an improved perception of 3D visuals over 2D, especially when adequate depth cues are provided. The state of modern hardware has made VR more affordable and available to wider audiences, at ever increasing resolutions of display than previously possible. It is therefore timely for research to be conducted to compare the perception of dynamic linear projections across display type.
...
1. **How can UCS be in 1- and 2D projections within an animation-specific framework?**\
(Work in progress, chapter \ref{ch:workinprogress}.)\
Following the theory laid out in @cook_manual_1997, a new UCS algorithm is devised for use within animation-specific frameworks. This enables fine control of exploring the local structure of data in 2D embeddings and sets the foundation to be used in the remaining objectives.

2. **Does 2D UCS provide benefits over alternatives?**\
...
(Future work, chapter \ref{ch:future_work}.)\
The UCS algorithm needs to be extended to a third dimension and the rendering of 3D dynamic projections and sectioning for use across mixed reality display devices. This will involve the addition of a new angle and it's control to the projection space, reference frame, and  manipulation space (now 4D) to the current algorithm.

4. **Does UCS in 3D displays provide perception benefits over 2D displays?**\
(Future work, chapter \ref{ch:future_work}.)\
The addition of a stereoscopic 3rd dimension has previously shown improved perception over 2D in other visualization including embedded spaces. The extension of UCS into 3D should be used to explore user perception across display devices (including a 2D standard monitor, 3D head tracked monitor, and 3D head-mounted display).

**Contributions:**

The contributions and thus concluding scope of this study can then be summarized with:

1. An algorithm for use in 1- and 2D animation frameworks.
2. A comparison study of benchmark data sets across dynamic and static projection techniques.
3. An algortithm for UCS and dynamic surface tours in 3D.
4. Quantification of the speed, accuracy, and correctness of USC across display device.

## Methodology

This research is interdisciplinary; it stems from a linear dimension reduction technique developed by statisticians and extended with information technology into 3D including VR technologies, with applications in high energy physics identified[@cook_dynamical_2018]. The research is correspondingly supervised by experts in these fields.
